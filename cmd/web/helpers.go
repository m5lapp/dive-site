...
func (app *app) serverError(w http.ResponseWriter, r *http.Request, err error) {
	...
	if app.debug {
		responseBody = fmt.Sprintf("%s\n\n%s\n\n%s", responseBody, err.Error(), trace)
	}
...
func (app *app) newTemplateData(r *http.Request) (templateData, error) {
	...
	data := templateData{
		...
		FlashInfo:       app.sessionManager.PopString(r.Context(), "flashInfo"),
		FlashSuccess:    app.sessionManager.PopString(r.Context(), "flashSuccess"),
		FlashWarning:    app.sessionManager.PopString(r.Context(), "flashWarning"),
